...
**drain 期間 dispatcher 不得開始新工作（keystone 定案）**：`drain_unstarted()` 進去先把 dispatcher latch 住（不再讓任何 job 轉入 started）再掃描，兩者在同一次持鎖內完成；run loop 標 started 前取同一把鎖。缺這個 latch，poller 最後一輪 dispatch 出去的 job 會在「poller.join() 返回」到「掃描」之間被 run loop 撿走，變成 shutdown 之後才開工、退場時被丟棄、再靠 reclaim 補 attempts，正是 drain 要避免的折損。已 in-flight 的 job 不受 latch 影響（剩餘 case 必須跑完，否則永不 complete）。
