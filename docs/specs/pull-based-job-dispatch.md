...
**drain 期間 dispatcher 不得開始新工作（keystone 定案）**：drain 的順序是 `poller.stop()` → **`dispatcher.stop_accepting()`（latch）** → 等 poller 結束 → 掃描未開始的 job。latch 必須在「等 poller」之前：poller 可能正卡在 prep，等待期間 prep 成功就會 dispatch，此時 dispatcher 若仍在收件，run loop 會把它轉成 started，之後的掃描就放過它——變成 shutdown 之後才開工、退場時被丟棄、再靠 reclaim 補 attempts，正是 drain 要避免的折損。掃描留在 poller 結束之後，才看得到所有 late dispatch；`handle()` 的狀態發布與入列一併收進 `state_lock`，掃描持鎖時看到的要嘛是整個 job 要嘛沒有。已 in-flight 的 job 不受 latch 影響（剩餘 case 必須跑完，否則永不 complete）。
